...
function Services(props) {
    ...
    function fetchService() {
        http.get(
            `/services/${serviceid.servicetype}/${serviceid.serviceid}`
        ).then((res) => {
            setData(res.data);
    ...
    function submitRequest() {
            ...
            for (let i = 0; i < data?.service?.dropdowns.length; i++) {
                console.log(
                    document.getElementById(
                        `${data?.service?.dropdowns[i].title}`
                    )
                );
                dropdowns.push({
        ...
        if (data?.checks[0]?.needs_approval_from) {
            fData.append(
                "needs_approval_from",
                data?.checks[0]?.needs_approval_from
            );
        }
        if (data?.checks[0]?.opened_for) {
            fData.append(
                "opened_for",
                document.getElementById("opened_for").value
            );
        }

        http.post("/submitservicerequest", fData)
            .then((res) => {
                console.log(res);
    ...
    return (
        <div className="sshow">
            <div className="sshow_top">
                <div className="sshow_top_imgCon">
                    {data?.service?.image && (
                        <img
                            className="sshow_top_img"
                            src={`/app/public/services${data?.service?.image}`}
                        ></img>
                    )}{" "}
                </div>
                <div className="sshow_top_text">
                    <div className="sshow_top_text_h">
                        {data?.service?.title}
                    </div>
                    <div className="sshow_top_text_p">
                    ...
                    {questionsAnswers.map((doc, index) => (
                        <div className="sshow_block">
                            <div className="sshow_checks_item">
                                <div className="sshow_checks_item_h">
                                    {doc.title}
                                </div>
                                <input
                                    type="text"
                        ...
                        data?.checks[0]?.needs_approval_from && (
                            <div className="sshow_block">
                                <div className="sshow_checks_item">
                                    ...
                                    <div className="sshow_checks_item_p">
                                        {
                                            data?.checks[0]
                                                ?.needs_approval_from_name.name
                                        }
                                    </div>
                                </div>
                            </div>
